...
export function useDashboardData() {
  ...
  const derived = useMemo(() => {
    ...
    const futureScheduled = scheduled
      .filter(r => (r.callTime || r.time) && isFuture(r.callTime || r.time))
      .sort((a, b) => new Date(a.callTime || a.time) - new Date(b.callTime || b.time));

    const todayAll = reminders
      .filter(r => (r.callTime || r.time) && isToday(r.callTime || r.time))
      .sort((a, b) => new Date(a.callTime || a.time) - new Date(b.callTime || b.time));
    ...
    const history = reminders
      .filter(r => {
        ...
        const callTimeVal = r.callTime || r.time;
        return (
          status === 'CALLED' ||
          status === 'MISSED' ||
          status === 'IN INVENTORY' ||
          (status === 'SCHEDULED' && callTimeVal && isPast(callTimeVal))
        );
      })
      .sort((a, b) => new Date(b.callTime || b.time) - new Date(a.callTime || a.time)) // most recent first
